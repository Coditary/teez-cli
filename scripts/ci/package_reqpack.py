...
def build_metadata(
    ...
    return {
        ...
        "release": 1,
        "revision": 0,
        "summary": "Unified test runner CLI",
        "description": "teez (test everything easy) — CLI frontend for discovering and running tests across ecosystems.",
        "license": "MIT",
        "architecture": arch,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Package teez as ReqPack .rqp archive")
    ...
    with tempfile.TemporaryDirectory(prefix=f"teez-rqp-{args.platform}-{args.arch}-", dir=output_dir) as temp_dir:
